...
| `auto-cluster` | After every successful lark-bd-poller cycle | salonx-flightdeck | enabled | 2026-05-12 | Fires computeFreshThemes() at the end of each successful BD poller cycle. Reads BD + Dev rows, clusters new arrivals into existing themes (or mints up to 2 brand-new themes per call, drift-promotes to from-scratch when prior incremental minted any). Disable here to make the next cycle skip the cluster step without touching BD ingestion. Phase 8 verification 2026-05-12: live on hubbibi, dev_count > 0 on multiple themes (push tickets bucketed), KILLSWITCH gating verified end-to-end. |
